...
df_sss <- readxl::read_excel(
  paste("../data/BASE COVID-19-", hoy.ss, "-IDCL-BM.xlsx", sep='')
) %>%
  select(
    EDAD, SEXO, `MUN RES`, `SER MED`, CONTACTO, VIAJE,
    `TIPO ATN`, `CLASIFICACIÓN`, `FECHA DE CLAS`, 
    `FECHA DE CONF`, `RESULTADO`, `INI SINT`
  ) %>%
  rename(
    Edad = EDAD, Sexo = SEXO, Municipio = `MUN RES`, 
    Servicio.Medico = `SER MED`, Contacto =  CONTACTO, 
    Viaje = VIAJE, Tipo.Atencion = `TIPO ATN`, 
    Final = `CLASIFICACIÓN`, Fecha.Final = `FECHA DE CLAS`, 
    Fecha = `FECHA DE CONF`, Resultado = `RESULTADO`, Fecha.Inicial = `INI SINT`
  ) %>%
  mutate(
    Fecha = as.Date(Fecha),
    Fecha.Final = as.Date(Fecha.Final),
    Fecha.Inicial = as.Date(Fecha.Inicial)
  )
...
df_municipios <- df_sss %>%
  ...
  dplyr::summarise(
    Confirmados = n(),
    Importados = sum(as.numeric(Viaje == "SI"), na.rm = TRUE),
...
df_son_c <- data.frame(
  Estado = "Sonora",
  Confirmados = sum(df_municipios$Confirmados),
  Recuperados = sum(df_municipios$Recuperados),
  Decesos = sum(df_municipios$Decesos)
)
...
plotly::plot_ly(data = df_s %>% filter(Fecha >= "2020-03-15")) %>%
...
#)
gen.per <- round( 100 * last(df_mio$Mujeres) / last(df_mio$Confirmados))
valueBox(
  value = gen.per, 
...
letalidad <- round(100 * last(df_mio$Decesos) / last(df_mio$Confirmados))
...
- *Confirmados*: cantidad de pruebas que resultaron positivas a la enfermedad (SARS-CoV2)
...
- Tasa de Letalidad = 100 * Decesos / Confirmados
